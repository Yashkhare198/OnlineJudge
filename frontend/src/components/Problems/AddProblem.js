...
const AddProblemForm = () => {
  ...
  // Function to fetch the next problem number
  const fetchNextProblemNumber = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/problems');
      const nextProblemNo = response.data.nextProblemNo;
      setProblemNo(nextProblemNo);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    }, 100); // Fetch every 10 seconds (adjust the interval as needed)
  ...
  const handleAddProblem = async () => {
    try {
      ...
      if (response.status === 201) {
        ...
        setTestCases('');
        setLevel('');
        // Fetch the next problem number again after adding a problem
        fetchNextProblemNumber();
      } else {
